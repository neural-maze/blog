...
summary: "A funny weekend project where I set two LLMs to play chess."
description: "A funny weekend project where I set two LLMs to play chess."
...
So, the other day I was playing around in Twitter when something got my attention: **LLMs playing Street Fighter!**

![Alt text][image-1]

At first, I thought it was some kind of joke, some of these videos that populate Twitter like an epidemic for a couple
of days but ... it was real. So after reading a couple of articles and the [Matthew Berman](https://www.youtube.com/watch?v=CGV0MlnOd30&ab_channel=MatthewBerman)
video on the topic, I thought it would also be cool to set the same kind of "tournament" but using chess.

What will happen if we put MistralAI and OpenAI LLMs in front of each other? What type of openings would gpt-3.5-turbo
use? Would Mistral use the Sicilian defense? Or something more creative? Would it be a complete chaos, where the LLMs
don't even respect the rules of chess? (being honest? this was my personal bet 😂) 

Driven by curiosity, I decided to implement a proof of concept that would allow us to test the quality of LLMs as GMs.
I've divided the process I followed into three parts (which corresponds to the three following sections)

## Langchain Dialogue Agents

The idea is really simple: create two Langchain Agents (what a surprise!!) that, instead of talking to each other, 
will exchange chess moves. At the end of the interaction we should have something similar to a PGN (Portable Game File) content, like
the one in the figure below.

<p align="center">
  <img src="img/chess_algebraic_notation.png" />
</p>

In this case, I'm going to create an agent powered by **GPT-4** and another one by **Mistral Large**. 
...
[image-1]: img/street_fighter.png "Mistral small vs Mistral medium in Street Fighter"
[image-2]: img/chess_algebraic_notation.png "An example of chess algebraic notation"
